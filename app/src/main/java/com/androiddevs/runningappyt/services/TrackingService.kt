...
import com.androiddevs.runningappyt.other.Constants.DELAY_TIME
...
typealias Polyline = MutableList<LatLng>//A simple Polyline ...set of coordinates which are joined by a line segment consecutive ones*
typealias Polylines = MutableList<Polyline>//list of Polyline
...
class TrackingService:LifecycleService() {
    ...
    //in Api to request Current Location Updates
    @Inject
    ...
    //since both liveDatas are created we have to put some value into it
    ...
    //it uses fusedLocationProvoderClient to request Immediate location updates
    //nd uh can specify the interwals of updates nd so on
    ...
    private fun updateLocationTracking(isTracking: Boolean) {
        if(isTracking) {
            ...
            //if we r not tracking then we dot request Location U[dates
            fusedLocationProviderClient.removeLocationUpdates(locationCallback)
        }
    }

    //Location Calback is used to provide New Location Updates regularly
    ...
    //once we start tracking...this fun adds new coordinates to the polyline list
    ...
    //this is used when suppose we stop and resume the service so that the polylines are not seperate
    ...
    private fun startForegroundService(){
        ...
        //here lay Notification Builder code(Dagger)
    ...
    //here lay PendingIntent onResult(Dagger)
